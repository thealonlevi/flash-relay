#!/usr/bin/env bash
# research/optimizer/score.sh — the FIXED referee. Scores the CURRENT working-tree io_uring
# relay (research/gate/cmd/relay-uring + research/gate/internal/uring) on a CPU-bound loopback churn
...
PORTS=$(python3 research/optimizer/pick-ports.py "${PORT_BASE:-40000}" 6 100) \
...
# --- gate 1: build ----------------------------------------------------------
if ! CGO_ENABLED=0 go build -o bin/relay-uring ./research/gate/cmd/relay-uring 2>/tmp/opt-build.log; then
  log "build FAILED -> score 0"; emit '{"score":0,"reason":"build_fail"}'
fi
CGO_ENABLED=0 go build -o bin/sink ./research/gate/cmd/sink 2>/dev/null
CGO_ENABLED=0 go build -o bin/loadgen ./research/gate/cmd/loadgen 2>/dev/null
CGO_ENABLED=0 go build -o bin/bulkgen ./research/gate/cmd/bulkgen 2>/dev/null
BIN=$(pwd)/bin
RSTAT=/tmp/opt-relay.stats; SSTAT=/tmp/opt-sink.stats; rm -f "$RSTAT" "$SSTAT"

# ===========================================================================
# OBJECTIVE = bytes_per_cpu : B4 bulk-throughput arm. SCORE = verified relayed
# bytes per relay instruction (x1e6). Stresses the data plane (recv/send/splice).
# ===========================================================================
if [ "$OBJECTIVE" = "bytes_per_cpu" ]; then
  ESTAT=/tmp/opt-echo.stats; rm -f "$ESTAT"
  taskset -c "$SINK_CPU" "$BIN/sink" -addr 127.0.0.1:$TEPORT -echo -statsfile "$ESTAT" >/tmp/opt-echo.log 2>&1 & SINK_PID=$!
  sleep 0.4
  ...
  # smoke
  "$BIN/bulkgen" -relay 127.0.0.1:$TPORT -conns 4 -chunk 65536 -warmup 1s -duration 1s >/tmp/opt-bsmoke.json 2>/dev/null
  sb=$(python3 -c "import json;print(json.load(open('/tmp/opt-bsmoke.json'))['bytes'])" 2>/dev/null || echo 0)
...
# ===========================================================================
# OBJECTIVE = instr_pc (default) : churn arm. SCORE = 1e9/instr_per_conn.
# ===========================================================================
