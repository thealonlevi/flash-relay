#!/usr/bin/env bash
# research/optimizer/loop.sh — the autonomous hill-climb. One mutation per iteration:
#   MUTATE:  `claude -p` makes ONE focused edit to the io_uring relay hot path.
#   MEASURE: research/optimizer/score.sh measures the objective(s) + per-arm anti-cheat gate.
...
# Runs on a dedicated branch so main stays clean. Stop: touch research/optimizer/results/STOP
set -uo pipefail
cd "$(dirname "$0")/../.."
...
measure(){ local r; r=$(OBJECTIVE="$1" PORT_BASE="$2" bash research/optimizer/score.sh 2>>"$RESULTS_DIR/loop-logs/score.log"); echo "$(echo "$r"|getf score) $(echo "$r"|getf reason)"; }
...
while [ "$iter" -lt "$MAX_ITERS" ]; do
  ...
  out=$(timeout "$ITER_TIMEOUT" env IS_SANDBOX=1 claude -p "$(cat research/optimizer/optimizer-prompt.md)" \
        ${SID:+--resume "$SID"} ${OPT_MODEL:+--model "$OPT_MODEL"} \
        --append-system-prompt-file research/optimizer/optimizer-system.md \
  ...
  case "$decision" in
    ...
    promote)
      ...
      if [ "${RUN_GATE_SUITE:-1}" = 1 ]; then
        gs=$(bash research/optimizer/gate-suite.sh 2>>"$RESULTS_DIR/loop-logs/gate-suite.log")
