#!/usr/bin/env bash
# research/optimizer/gate-suite.sh — the PROMOTION gate suite. The inner loop maximizes ONE
...
PORTS=$(python3 research/optimizer/pick-ports.py "${PORT_BASE:-43000}" 4 100) \
...
CGO_ENABLED=0 go build -o bin/relay-uring ./research/gate/cmd/relay-uring 2>/dev/null || emit '{"pass":0,"reason":"build_fail"}'
...
CGO_ENABLED=0 go build -o bin/holdgen ./research/gate/cmd/holdgen 2>/dev/null
BIN=$(pwd)/bin
...
if [ -z "$CROSS_BASE" ]; then
  ...
  CROSS_SCORE=$(OBJECTIVE=$OTHER PORT_BASE=44000 bash research/optimizer/score.sh 2>>/tmp/gs-cross.log \
