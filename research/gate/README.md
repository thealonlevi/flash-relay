...
```
CGO_ENABLED=0 go build ./research/gate/...     # pure Go, no cgo
go test ./research/gate/internal/uring/        # ring smoke tests (recv/send + accept)
```

## Run the gate (root: needs perf + taskset)

```
sudo bash research/gate/harness/gate.sh                      # headline (CPU-isolation)
...
REQLEN REPLYLEN AUTHCPU`. Results land in `research/gate/results/<timestamp>/` with a
