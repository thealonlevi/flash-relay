...
for c in sink loadgen loadgend; do
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /tmp/$c ./research/gate/cmd/$c
done
...
for c in sink loadgen loadgend; do CGO_ENABLED=0 go build -o /usr/local/bin/$c ./research/gate/cmd/$c; done
```
...
```bash
sudo env B2=$BOX2_IP BOX1_IP=$BOX1_IP bash research/gate/harness/run-2box.sh
# realistic-dial parking test:
sudo env B2=$BOX2_IP BOX1_IP=$BOX1_IP REALISTIC=1 INFLIGHT=8000 bash research/gate/harness/run-2box.sh
```
...
1. On **box 1**, start the SUT-side harness (it starts the relay + runs perf and
   ...
   sudo env BUILD=uring   SINK=$BOX2_IP:$SPORT RPORT=$RPORT DUR=10 REPS=5 \
     bash research/gate/harness/run-sut.sh
   # later, second pass:
...
```bash
python3 research/gate/harness/combine-2box.py results/2box-<ts> \
